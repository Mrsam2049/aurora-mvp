...
router.post('/activate', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (authError) {
      // No filtramos el mensaje interno de Supabase al cliente (evita
      // info-disclosure / enumeración). Lo dejamos solo en logs de servidor.
      if (process.env.NODE_ENV !== 'production') {
        console.error('[activate] createUser error:', authError.message);
      }
      return res.status(500).json({ error: 'No se pudo activar la cuenta. Inténtalo de nuevo.' });
    }
